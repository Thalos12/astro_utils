...
def gen_png(*args):
    ...
    mlab.points3d(data[::res,0], data[::res,1], data[::res,2], rho, colormap='hot', mode='sphere', vmax=dmax, vmin=dmin, figure=mfig)  # posiziono i punti in 3d
    ...
    mlab.axes(nb_labels=5, x_axis_visibility=False, z_axis_visibility=False, figure=mfig)
    outline = mlab.outline(figure=mfig)
    outline.outline_mode = 'cornered'
    # mlab.axes(y_axis_visibility=False, z_axis_visibility=False)
    mlab.view(-45.0, 90.0, distance='auto', figure=mfig)  # imposto l'angolo di visione dell'insieme dei dati
    # mlab.show()
    mlab.savefig(f_name+'.png', figure=mfig)
    mlab.close(mfig)  # chiudo la figura per liberare memoria
